...
export class MemoryMediaObjectRepository implements MediaObjectsRepository {
  ...
  async putMediaObject(mediaObject: MediaObject): Promise<MediaObject> {
    const index = this.mediaObjects.findIndex(
      (findMediaObject) => findMediaObject.objectId === mediaObject.objectId
    );
    if (index === -1) {
