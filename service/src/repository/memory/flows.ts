...
export class MemoryFlowsRepository implements FlowRepository {
  ...
  async listFlows(filters?: ListFlowsFilters): Promise<ListFlowsResponse> {
    ...
    if (filters?.flowFormat) {
      filteredFlows = filteredFlows.filter(
        ({ format }) => format === filters.flowFormat
      );
    }
    if (filters?.codec) {
      filteredFlows = filteredFlows.filter(
        ({ codec }) => codec === filters.codec
      );
    }
    if (filters?.label) {
      filteredFlows = filteredFlows.filter(
    ...
    if (filters?.frameWidth) {
      filteredFlows = filteredFlows.filter((flow) => {
        if (
          flow.format === FormatUrn.VIDEO ||
          flow.format === FormatUrn.IMAGE
        ) {
    ...
    if (filters?.limit) {
      if (filteredFlows.length > filters.limit) {
        nextPageToken = this.encodePageToken(
          filteredFlows[filters.limit - 1].flowId
        );
      }
  ...
  async getFlowById(flowId: string): Promise<Flow | null> {
    return (
      this.flows.find(({ flowId: findFlowId }) => findFlowId === flowId) ?? null
    );
  }

  async putFlow(flow: Flow): Promise<Flow> {
    const indexToUpdate = this.flows.findIndex(
      ({ flowId: findFlowId }) => findFlowId === flow.flowId
    );
    if (indexToUpdate === -1) {
