...
export class ServiceRoutes extends Routes {
  private readonly serviceRepository = inject(serviceRepositoryToken);

  constructor(backends: BackendManager) {
    super(undefined, backends);
  ...
  private async get(_: Request, res: Response<GetServiceResponse>) {
    const { name, description } = await this.serviceRepository.getService();
    res.json({
      name,
      description,
      api_version: "1.0",
  ...
  private async getStorageBackends(_: Request, res: Response<StorageBackends>) {
    res.json(
      await Promise.all(
        this.backends!.getBackends().map(async (backend) => {
